<template>
  <div class="app-container">
    ...
    <el-table style="width: 100%"
      ...
      <el-table-column prop="gradeName" label="Grade名称"></el-table-column>
      <el-table-column prop="listPrice" label="展示价格"></el-table-column>
      <el-table-column prop="salePrice" label="销售价格"></el-table-column>
      <el-table-column prop="gradeAge" label="适合的年龄"></el-table-column>

      <el-table-column prop="gradeLink" label="Grade链接"></el-table-column>
      <el-table-column prop="status" label="Grade状态"></el-table-column>
      <el-table-column prop="gradeDesc" label="Grade描述"></el-table-column>

      <el-table-column prop="gradeCover" label="Grade图片">
        <template slot-scope="scope">
          <img :src="scope.row.gradeCover" style="width: 100px;height: 100px;display: block;" />
        </template>
      </el-table-column>
      <el-table-column prop="gradeVideo" label="GradeVideo">
        <template slot-scope="scope">
          <video :src="scope.row.gradeVideo"  controls preload="auto"  style="width: 100px;height: 100px;display: block;" />
        </template>
      </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      fetchData(current) {
        ...
        courseApi.queryGrade(this.tableQuery, this.tablePage).then(res => {
      ...
      handleDelete(idx, row) {
        this.$confirm('您确定要永久删除该Grade？', '提示', confirm).then(() => {
          courseApi.deleteGradeById({ gradeId: row.id }).then(res => {
      ...
      handleCreate() {
        this.$router.push({
          path: '/mengya/grade/addGrade'
        })
      },
      handleUpdate(idx, row) {
        this.$router.push({
          path: '/mengya/grade/editGrade/' + row.id
        })
      }
    }
  }
