  ...
  <div class="app-container">
    ...
    <el-row>
      ...
      <el-tooltip class="item" content="搜索" placement="top" >
        <el-button icon="el-icon-search" circle @click="fetchData(1)" v-perm="'b:staff:query'"></el-button>
      </el-tooltip>
    </el-row>
    <div style="margin-bottom: 30px;"></div>
    <el-button type="primary" icon="el-icon-plus" size="mini" @click="handleCreate" v-perm="'b:staff:add'">{{textMap.create}}</el-button>
    <div style="margin-bottom: 30px;"></div>
    <!--列表-->
  ...
  export default {
    ...
    methods: {
      ...
      fetchData(current) {
        ...
        userApi.queryStaff(this.tableQuery, this.tablePage).then(res => {
      ...
      updateData() {
        this.$refs['dataForm'].validate((valid) => {
          ...
          userApi.updateStaff(tempData).then(res => {
      ...
      invokeUpdateUserRolesApi(){
        userApi.updateStaffRoles(this.updateUserRolesData).then(res => {
      ...
      handleDelete(idx, row) {
        this.$confirm('您确定要永久删除该用户？', '提示', confirm).then(() => {
          userApi.deleteStaff({uid: row.uid}).then(res => {
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (!valid) return;
          userApi.addStaff(this.temp).then((res) => {
