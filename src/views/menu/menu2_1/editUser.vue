<template>
  <div class="app-container">
    ...
    <el-form :rules="rules" ref="dataForm" :model="temp" label-position="left" label-width="120px">
      ...
      <el-form-item label="用户编号" prop="userId" :required="true">
        <el-col :span="12">
      ...
      <el-row>
        <el-col :span="8">
          <el-form-item label="性别:" prop="gender" >
            <el-select v-model="temp.gender"  :disabled="true" >
      ...
      <el-form-item label="用户头像:">
        <template slot-scope="scope">
          <img :src="temp.headImage" style="width: 300px;height: 300px;display: block;" />
        </template>

      </el-form-item>

      <div style="margin-top: 30px; margin-bottom: 20px"><span>修改用户头像</span>
        <el-upload
          :action=uploadDomain
  ...
  export default {
    ...
    methods: {
      ...
      fetchData(userId) {
        ...
        userApi.userDetail(userId).then(response => {
          const user = response.data.user
          this.temp.userId = user.userId
          this.temp.slogon = user.slogon
          this.temp.userName = user.userName
          this.temp.nickname = user.nickname
          this.temp.mobileNo = user.mobileNo
          this.temp.email = user.email
          this.temp.description = user.description

          this.temp.gender = user.gender
          this.temp.birthday = user.birthday
          this.temp.headImage = user.headImage
          this.temp.roleId = user.roleId
        }).catch(err => {
          console.log(err)
        })
      },
